...
<html>
  ...
  <body>
    ...
    <form action="graduatedb.php" method="post">
      First Name* <br> <input type="text" name="fname"> <br>
      Last Name* <br> <input type="text" name="lname"><br>
      Year of Graduation from ACI*<br> <input type="text" name="yograd"><br>
      Field* <br>
      <input type="radio" name="field" value="tech">tech<br>
      <input type="radio" name="field" value="sci">sci<br>
      <input type="radio" name="field" value="eng">eng<br>
      <br>
      Field of Study* <br>
...
//check if all required fields have input and assing the variables.
      if(empty($_POST['fname'])) {echo "<script>alert('first name');</script>";} else {
        $fname = $_POST['fname'];
...
//checked and assigned
    /*the graduate*/  $thegraduate = "INSERT INTO graduates (fname, lname, yograd) VALUES ('$fname', '$lname', $yograd) ";
                      if($mysqli->query($thegraduate) === 1) {
                        $selectstuff = $mysqli->query("SELECT fields.id AS fieldsid, fieldofstudy.id AS fieldofstudyid, graduates.id AS graduatesid
                                              FROM fields, fieldofstudy, graduates WHERE (field = '$field' AND fostud = '$fostud' AND fname = '$fname' AND lname = '$lname' and yograd = $yograd)");

                                              if ($selectstuff->rowCount() == 0 ) {
                                              echo "No results";
                                              } else {
                                              while($rows = $selectstuff->fetch(PDO::FETCH_ASSOC)) {
                                                $fieldsid = $rows['fieldsid'];
                                                $fieldofstudyid = $rows['fieldofstudyid'];
                                                $graduatesid = $rows['graduatesid'];
                                                  $matchfield = "INSERT INTO gfield (g_id ,field_id, fostud_id) VALUES ($graduatesid, $fieldsid, $fostudid)";
